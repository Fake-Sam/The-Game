...
public class GameServerUDP extends GameConnectionServer<UUID> 
{
	...
	public void processPacket(Object o, InetAddress senderIP, int senderPort)
	{
		...
		{	// JOIN -- Case where client just joined the server
			...
			{	UUID clientID = UUID.fromString(messageTokens[1]);
				...
				sendCreateMessages(clientID, pos, type);
			...
			{	UUID clientID = UUID.fromString(messageTokens[1]);
				...
				String type = messageTokens[6];
				sendDetailsForMessage(clientID, remoteID, pos, type);
			}
	...
	public void sendCreateMessages(UUID clientID, String[] position, String type)
	{	try 
	...
	public void sendDetailsForMessage(UUID clientID, UUID remoteId, String[] position, String type)
	{	try 
	...
	public void sendYawMessages(UUID clientID, float angle) {
		try {
			String message = "yaw," + clientID.toString();
			message += "," + angle;
			forwardPacketToAll(message, clientID);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void sendPitchMessages(UUID clientID, float angle) {
		try {
			String message = "pitch," + clientID.toString();
			message += "," + angle;
			forwardPacketToAll(message, clientID);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void sendDiedMessages(UUID clientID) {
		try {
			String message = "died," + clientID.toString();
			forwardPacketToAll(message, clientID);
		} catch (IOException e) {
			e.printStackTrace();
		}
