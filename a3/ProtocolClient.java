...
public class ProtocolClient extends GameConnectionClient
{
		...
		if(messageTokens.length > 0)
		{
			...
			{	if(messageTokens[1].compareTo("success") == 0)
				{	System.out.println("join success confirmed");
					...
					sendCreateMessage(game.getPlayerPosition(), avatarType);
				}
			...
			{	// create a new ghost avatar
				...
				String type = messageTokens[5];

				try
				{	ghostManager.createGhostAvatar(ghostID, ghostPosition, type);
				}	catch (IOException e)
			...
			if (messageTokens[0].compareTo("wsds") == 0)
			{
				...
				// Parse out the id into a UUID
				UUID ghostID = UUID.fromString(messageTokens[1]);
				sendDetailsForMessage(ghostID, game.getPlayerPosition(), avatarType);
			}
			...
			if (messageTokens[0].equals("yaw")) {
				UUID ghostID = UUID.fromString(messageTokens[1]);
				float angle = Float.parseFloat(messageTokens[2]);
				ghostManager.updateGhostYaw(ghostID, angle);
			}
			if (messageTokens[0].equals("pitch")) {
				UUID ghostID = UUID.fromString(messageTokens[1]);
				float angle = Float.parseFloat(messageTokens[2]);
				ghostManager.updateGhostPitch(ghostID, angle);
			}
			if (messageTokens[0].equals("died")) {
				UUID ghostID = UUID.fromString(messageTokens[1]);
				ghostManager.handleGhostDied(ghostID);
			}
		}
	}
	...
	public void sendJoinMessage(String avatarType)
	{	try 
	...
	public void sendCreateMessage(Vector3f position, String type)
	{	try 
	...
	public void sendDetailsForMessage(UUID remoteId, Vector3f position, String type)
	{	try 
	...
	public void sendYawMessage(float angle){
		try {
			String message = "yaw," + id.toString();
			message += "," + angle;
	...
	public void sendPitchMessage(float angle) {
		try {
			String message = "pitch," + id.toString();
			message += "," + angle;
	...
	public void sendDiedMessage() {
		try {
			String message = "died," + id.toString();
