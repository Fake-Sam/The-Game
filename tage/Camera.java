...
public class Camera
{
	...
	/** adds angle to current yaw */
	public void yaw(float angle) {
		setN(getN().rotateAxis(angle, getV().x, getV().y, getV().z));
		setU(getU().rotateAxis(angle, getV().x, getV().y, getV().z));
	}

	/** adds angle to current pitch */
	public void pitch(float angle) {
		setN(getN().rotateAxis(angle, getU().x, getU().y, getU().z));
		setV(getV().rotateAxis(angle, getU().x, getU().y, getU().z));
	}
}