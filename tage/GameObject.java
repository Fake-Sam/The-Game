...
public class GameObject
{
	...
	*/
	...
	/** add angle to current pitch */
	public void pitch(float angle) {
		Matrix4f r = getWorldRotation();
		r.rotateX(angle);
		setLocalRotation(r);
	}

	/** pitch the object by its own v axis in radians */
	public void pitchLocal(float rads) {
		Matrix4f objMatrix = getLocalRotation();
		objMatrix.rotate(rads, 1, 0, 0);
		setLocalRotation(objMatrix);
	}

	/** yaw the object by the world's y axis in radians */
	public void yawWorld(float rads) {
		Matrix4f worldRotate = (new Matrix4f()).identity();
		worldRotate.rotate(rads, 0, 1, 0);
		worldRotate.mul(getLocalRotation());
		setLocalRotation(worldRotate);
	}

	/** yaws the object by its own n axis in radians */
	public void yawLocal(float rads) {
		Matrix4f objMatrix = getLocalRotation();
		objMatrix.rotate(rads, 0, 1, 0);
		setLocalRotation(objMatrix);
	}

	/** rolls the object by its own u axis in radians */
	public void rollLocal(float rads) {
		Matrix4f objMatrix = getLocalRotation();
		objMatrix.rotate(rads, 0, 0, 1);
		setLocalRotation(objMatrix);
	}
